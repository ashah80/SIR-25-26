...
def get_video_and_audio(path, get_meta=False, start_sec=0, end_sec=None):
    ...
    # --- Robust FPS handling ---
    if 'video_fps' in meta and meta['video_fps'] is not None:
        fps = float(meta['video_fps'])
    else:
        # Infer FPS from timestamps if missing
        if rgb.shape[0] > 1 and 'video_timebase' in meta:
            # timebase = seconds per tick
            timebase = float(meta['video_timebase'])
            duration = (rgb.shape[0] - 1) * timebase
            fps = rgb.shape[0] / max(duration, 1e-6)
        else:
            # fallback: assume FakeAVCeleb standard
            fps = 25.0
    ...
    if 'video_fps' in meta and meta['video_fps'] is not None:
        ...
        video_fps = 25.0  # FakeAVCeleb standard
    
    # Audio FPS (sample rate)
    if 'audio_fps' in meta and meta['audio_fps'] is not None:
        audio_fps = int(meta['audio_fps'])
    elif 'audio_timebase' in meta and meta['audio_timebase'] is not None:
        # audio_timebase = seconds per sample
        audio_fps = int(round(1.0 / float(meta['audio_timebase'])))
    else:
        audio_fps = 16000  # FakeAVCeleb standard
    
    meta = {
        'video': {'fps': [video_fps]},
        'audio': {'framerate': [audio_fps]},
    }
